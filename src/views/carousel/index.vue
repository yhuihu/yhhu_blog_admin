<template>
  <div class="common-table">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <img :src="scope.row.image" class="image-cell">
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <el-dialog title="增加轮播" :visible.sync="addVisible" width="30%">
      <el-form :model="form" label-width="80px">
        <el-form-item label="轮播次序">
          <el-input v-model="form.imageOrder" class="inner-item" />
        </el-form-item>
...
<script>
import { fetchCarouselList, deleteCarousel, addCarousel, updateCarousel } from '@/api/carousel'
  ...
  methods: {
    ...
    handleDelete() {
      ...
      this.$confirm(`此操作将永久删除该轮播，是否继续?`, '提示', {
        ...
        deleteCarousel(this.currRow.id).then(response => {
          // 更新数据
          this.fetchData()
          this.$message({
            type: 'success',
